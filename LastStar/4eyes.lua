...
if not getgenv().Network then
	...
	Network.RetainPart = function(Part) -- Retain part just configures ONE property. I just dumbed it down.
		assert(typeof(Part) and (Part:IsA("BasePart") or Part:IsA("MeshPart")))
		if not table.find(Network["Instances"],Part) then
			Part.CustomPhysicalProperties = PhysicalProperties.new(0,0,0,0,0)
			table.insert(Network["Instances"],Part)
		else
			--wxrn("[NETWORK]: Instance '"..Part.Name.."' is already being retained!")
		end
	end
	...
	Network["PartOwnership"]["Enable"] = coroutine.create(function() --creating a thread for network stuff
		if Network["PartOwnership"]["Enabled"] == false then
			...
			--wxrn("[NETWORK] PartOwnership Output : PartOwnership enabled.")
		else
			--wxrn("[NETWORK] PartOwnership Output : PartOwnership already enabled.")
		end
	end)
	Network["PartOwnership"]["Disable"] = coroutine.create(function()
		if Network["PartOwnership"]["Connection"] then
			...
			Network["PartOwnership"]["PreMethodSettings"] = {}
			Network["PartOwnership"]["Enabled"] = false
			--wxrn("[NETWORK] PartOwnership Output : PartOwnership disabled.")
		else
			--wxrn("[NETWORK] PartOwnership Output : PartOwnership already disabled.")
		end
	end)

	--wxrn("[NETWORK]: Successfully loaded.")
else
	--wxrn("[NETWORK]: Already loaded!")
end