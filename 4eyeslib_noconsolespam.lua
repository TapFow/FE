...
if not getgenv().Network then
	getgenv().Network = {
		...
		Output = {
			...
			Send = function(Type,Output,BypassOutput)
				if typeof(Type) == "function" and (Type == printuiconsole or Type == warnuiconsole or Type == erroruiconsole) and typeof(Output) == "string" and (typeof(BypassOutput) == "nil" or typeof(BypassOutput) == "boolean") then
				...
				elseif Network["Output"].Enabled == true then
					erroruiconsole(Network["Output"].Prefix.."Output Send Error : Invalid syntax.");
				end;
			end;
		};
	...
	Network["Output"].Send(printuiconsole,": Loading.")
			...
			if Network.CharacterRelative == true then
				...
				if Character and Character.PrimaryPart then
					...
					if Distance > 1000 then
						Network["Output"].Send(warnuiconsole,"RetainPart Warning : PartOwnership not applied to BasePart "..Part:GetFullName()..", as it is more than "..gethiddenproperty(LocalPlayer,"MaximumSimulationRadius").." studs away.")
						return false
					end
				else
					Network["Output"].Send(warnuiconsole,"RetainPart Warning : PartOwnership not applied to BasePart "..Part:GetFullName()..", as the LocalPlayer Character's PrimaryPart does not exist.")
					return false
				end
			end
			table.insert(Network["BaseParts"],Part)
			Part.CustomPhysicalProperties = PhysicalProperties.new(0,0,0,0,0)
			Network["Output"].Send(printuiconsole,"PartOwnership Output : PartOwnership applied to BasePart "..Part:GetFullName()..".")
			if ReturnFakePart == true then
				return FakePart
			end
		else
			Network["Output"].Send(warnuiconsole,"RetainPart Warning : PartOwnership not applied to BasePart "..Part:GetFullName()..", as it already active.")
			return false
		end
	end
	...
	Network["RemovePart"] = function(Part) --function for removing ownership of unanchored part
		...
		if Index then
			...
			Network["Output"].Send(printuiconsole,"RemovePart Output: PartOwnership removed from BasePart "..Part:GetFullName()..".")
		else
			Network["Output"].Send(warnuiconsole,"RemovePart Warning : BasePart "..Part:GetFullName().." not found in BaseParts table.")
		end
	end
	...
	Network["PartOwnership"]["Enable"] = coroutine.create(function() --creating a thread for network stuff
		if Network["PartOwnership"]["Enabled"] == false then
						...
						if Part:IsDescendantOf(workspace) then
							if Network.CharacterRelative == true then
								...
								if Character and Character.PrimaryPart then
									local Distance = (Character.PrimaryPart.Position - Part.Position).Magnitude
									if Distance > 1000 then
										Network["Output"].Send(warnuiconsole,"PartOwnership Warning : PartOwnership not applied to BasePart "..Part:GetFullName()..", as it is more than "..gethiddenproperty(LocalPlayer,"MaximumSimulationRadius").." studs away.")
										Lost = true;
										Network["RemovePart"](Part)
									end
								else
									Network["Output"].Send(warnuiconsole,"PartOwnership Warning : PartOwnership not applied to BasePart "..Part:GetFullName()..", as the LocalPlayer Character's PrimaryPart does not exist.")
								end
							end
			...
			Network["Output"].Send(printuiconsole,"PartOwnership Output : PartOwnership enabled.")
		else
			Network["Output"].Send(warnuiconsole,"PartOwnership Output : PartOwnership already enabled.")
		end
	end)
	Network["PartOwnership"]["Disable"] = coroutine.create(function()
		if Network["PartOwnership"]["Connection"] then
			...
			Network["PartOwnership"]["Enabled"] = false
			Network["Output"].Send(printuiconsole,"PartOwnership Output : PartOwnership disabled.")
		else
			Network["Output"].Send(warnuiconsole,"PartOwnership Output : PartOwnership already disabled.")
		end
	end)
	Network["Output"].Send(printuiconsole,": Loaded.")
end
